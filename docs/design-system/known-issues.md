...
階梯目前有 49 階，仍比成熟系統該有的多——三案是各自繪製的。
...
### Headline 的 `headline-md` / `headline-sm`（風險中）

`clamp(38px, 3.5vw, 54px)` 與 `clamp(36px, 3.2vw, 50px)` 相差 2–4px，
分別用於 B 案的資源與活動區塊。合併為單一 B 案 headline 步階是合理的設計決策，
但同樣會改變版面。
